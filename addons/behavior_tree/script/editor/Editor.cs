...
[Tool]
public partial class Editor : GraphEdit
{
	...
	public GraphData DGraphData { get; private set; } = new ();
	private readonly System.Collections.Generic.Dictionary<string, DGraphNode> _nodes = new ();
	private readonly System.Collections.Generic.Dictionary<string, PackedScene> _nodesScenes = new ()
	{
		{"RootNode", ResourceLoader.Load<PackedScene>("res://addons/behavior_tree/scenes/nodes/root_node.tscn")},
	};
	...
	private void OnCopyNodesRequest()
	{
		...
		var dGraphNodes = nodeSelected as DGraphNode[] ?? nodeSelected.ToArray();
		if (!dGraphNodes.Any()) return;
		
		_copyNodeData = new Array<Dictionary>();
		foreach (var node in dGraphNodes)
		{
	...
	private void OnConnectionToEmpty(StringName fromNode, long fromPort, Vector2 releasePosition)
	{
		...
		void OnNodeCreated(DGraphNode node)
		{
	...
	{
		// 更新检查器面板
		// Dialogue.DInspector.InspectObject(node, inspectorOnly:true);
	}
	...
	/// <summary>
	/// <para>Create and init graph node <see cref="DGraphNode"/>. If there has data, then deserialize node data.</para>
	/// </summary>
	/// <param name="nodeType"></param>
	/// <param name="data"></param>
	/// <typeparam name="T"></typeparam>
	public T CreateNode<T>(string nodeType, Dictionary data = null) where T : DGraphNode
	{
	...
	public void RemoveNode(DGraphNode node)
	{
		if (!IsInstanceValid(node)) return;
		// Can't remove start node.
		if (node.NodeType == "RootNode") return;
	...
	public DGraphNode GetNodeByName(string name)
	{
	...
	/// </summary>
	/// <returns><see cref="DGraphNode" /></returns>
	private IEnumerable<DGraphNode> GetNodeSelected()
	{
		var nodeSelected = new List<DGraphNode>();
	...
	/// </summary>
	/// <param name="data"></param>
	public void LoadData(GraphData data)
	{
		DGraphData = data;
	...
	public GraphData DumpsData()
	{
		...
		DGraphData.Nodes = data;
		
		DGraphData.Connection = GetConnectionList();
		return DGraphData;
	}

	#region delegate && event
	
	public delegate void NodeCreatedEventHandler(DGraphNode node);

	public event NodeCreatedEventHandler NodeCreated;
	
	public delegate void NodeRemovedEventHandler(DGraphNode node);
